...
def test_1inch_trade():
    """Test placing a trade on 1INCH/USD BUY signal"""
    logger = setup_logger('1inch_test')
    
    print("🚀 Testing 1INCH/USD BUY Signal Trade...")
    ...
    try:
        ...
        risk_amount = 5.0  # $5 risk for testing
        ...
        if amount < min_amount:
            amount = min_amount
            print(f"⚠️  Adjusted to minimum order size: {amount:.6f} 1INCH")
        
        # Ensure we don't exceed available balance
        max_order_value = balance * 0.8  # Use 80% of balance max
        max_amount = max_order_value / entry_price
        
        if amount > max_amount:
            amount = max_amount
            print(f"⚠️  Adjusted to maximum order size: {amount:.6f} 1INCH")
        
        print(f"📊 Position size: {amount:.6f} 1INCH")
        ...
        print(f"   Amount: {amount:.6f} 1INCH")
        print(f"   Entry: ${entry_price:.4f}")
        print(f"   SL: ${sl_price:.4f}")
        print(f"   TP: ${tp_price:.4f}")
        print(f"   Risk: ${risk_amount:.2f}")
        
        confirm = input("Do you want to place this 1INCH trade? (y/n): ").lower().strip()
        ...
        if confirm == 'y':
            ...
            if success:
                print("✅ 1INCH trade placed successfully!")
                print("📱 Check your Telegram for trade notification")
            else:
                print("❌ Failed to place 1INCH trade")
        else:
            print("❌ Trade cancelled by user")
            
    except Exception as e:
        print(f"❌ Error: {e}")
        logger.error(f"Error in 1INCH trade test: {e}")
    finally:
