...
def test_storj_trade():
    """Test placing a trade on STORJ/USD BUY signal"""
    logger = setup_logger('storj_test')
    
    print("🚀 Testing STORJ/USD BUY Signal Trade...")
    ...
    try:
        ...
        available_balance = balance * 0.03  # Use only 3% of balance for tight trades
        
        # Add a safety buffer for Kraken fees
        available_balance -= 2.0  # Reserve $2 for fees/slippage
        amount = available_balance / entry_price
        
        # Kraken may reject too many decimals (e.g., >8)
        amount = round(amount, 6)
        
        # Ensure minimum order size for Kraken (reduced for testing)
        min_order_value = 10.0  # $10 minimum
        min_amount = min_order_value / entry_price
        
        if amount < min_amount:
            amount = min_amount
            print(f"⚠️  Adjusted to minimum order size: {amount:.6f} STORJ")
        
        # Ensure we don't exceed available balance
        max_order_value = balance * 0.05  # Use 5% of balance max
        max_amount = max_order_value / entry_price
        
        if amount > max_amount:
            amount = max_amount
            print(f"⚠️  Adjusted to maximum order size: {amount:.6f} STORJ")
        
        print(f"📊 Position size: {amount:.6f} STORJ")
        ...
        print(f"   Amount: {amount:.6f} STORJ")
        print(f"   Entry: ${entry_price:.4f}")
        print(f"   SL: ${sl_price:.4f}")
        print(f"   TP: ${tp_price:.4f}")
        print(f"   Risk: ${risk_amount:.2f}")
        
        confirm = input("Do you want to place this STORJ trade? (y/n): ").lower().strip()
        ...
        if confirm == 'y':
            ...
            if success:
                print("✅ STORJ trade placed successfully!")
                print("📱 Check your Telegram for trade notification")
            else:
                print("❌ Failed to place STORJ trade")
        else:
            print("❌ Trade cancelled by user")
            
    except Exception as e:
        print(f"❌ Error: {e}")
        logger.error(f"Error in STORJ trade test: {e}")
    finally:
