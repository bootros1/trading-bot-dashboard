...
def test_zrx_trade():
    """Test placing a trade on ZRX/USD BUY signal"""
    logger = setup_logger('zrx_test')
    
    print("🚀 Testing ZRX/USD BUY Signal Trade...")
    ...
    try:
        ...
        if amount < min_amount:
            amount = min_amount
            print(f"⚠️  Adjusted to minimum order size: {amount:.6f} ZRX")
        ...
        if amount > max_amount:
            amount = max_amount
            print(f"⚠️  Adjusted to maximum order size: {amount:.6f} ZRX")
        
        print(f"📊 Position size: {amount:.6f} ZRX")
        print(f"🛑 Stop loss pips: {sl_pips:.4f}")
        print(f"🛑 Stop Loss: ${sl_price:.4f}")
        print(f"🎯 Take Profit: ${tp_price:.4f}")
        print(f"⚠️  Risk amount: ${available_balance:.2f}")
        
        # Ask for confirmation
        print(f"\n🤔 Ready to place BUY order:")
        print(f"   Amount: {amount:.6f} ZRX")
        print(f"   Entry: ${entry_price:.4f}")
        print(f"   SL: ${sl_price:.4f}")
        print(f"   TP: ${tp_price:.4f}")
        print(f"   Risk: ${available_balance:.2f}")
        
        confirm = input("Do you want to place this ZRX trade? (y/n): ").lower().strip()
        ...
        if confirm == 'y':
            ...
            if success:
                print("✅ ZRX trade placed successfully!")
                print("📱 Check your Telegram for trade notification")
            else:
                print("❌ Failed to place ZRX trade")
        else:
            print("❌ Trade cancelled by user")
            
    except Exception as e:
        print(f"❌ Error: {e}")
        logger.error(f"Error in ZRX trade test: {e}")
    finally:
