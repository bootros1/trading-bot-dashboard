...
def test_xlm_trade():
    """Test placing a trade on XLM/USD BUY signal"""
    logger = setup_logger('xlm_test')
    
    print("🚀 Testing XLM/USD BUY Signal Trade...")
    ...
    try:
        ...
        if amount < min_amount:
            amount = min_amount
            print(f"⚠️  Adjusted to minimum order size: {amount:.6f} XLM")
        ...
        if amount > max_amount:
            amount = max_amount
            print(f"⚠️  Adjusted to maximum order size: {amount:.6f} XLM")
        
        print(f"📊 Position size: {amount:.6f} XLM")
        ...
        print(f"   Amount: {amount:.6f} XLM")
        print(f"   Entry: ${entry_price:.4f}")
        print(f"   SL: ${sl_price:.4f}")
        print(f"   TP: ${tp_price:.4f}")
        print(f"   Risk: ${available_balance:.2f}")
        
        confirm = input("Do you want to place this XLM trade? (y/n): ").lower().strip()
        ...
        if confirm == 'y':
            ...
            if success:
                print("✅ XLM trade placed successfully!")
                print("📱 Check your Telegram for trade notification")
            else:
                print("❌ Failed to place XLM trade")
        else:
            print("❌ Trade cancelled by user")
            
    except Exception as e:
        print(f"❌ Error: {e}")
        logger.error(f"Error in XLM trade test: {e}")
    finally:
